/*
 * test.c - Suite de pruebas para las funciones del tablero
 */
...
void test_esJugadaValida() {
    ...
    assert(esJugadaValida(&t, 2, 3, 'N') == 1);
    assert(esJugadaValida(&t, 3, 2, 'N') == 1);
    assert(esJugadaValida(&t, 4, 5, 'N') == 1);
    assert(esJugadaValida(&t, 5, 4, 'N') == 1);
